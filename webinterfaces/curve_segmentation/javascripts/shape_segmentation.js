...
function get_closest_point_idx(event){
  ...
  if (HITstep == 1) {
    for(var i=0;i<points.length;i++){
      var d2 = (vx-get_x_vp(points[i]))*(vx-get_x_vp(points[i])) + (vy-get_y_vp(points[i]))*(vy-get_y_vp(points[i]));
      if(d2 < min_dist){
        min_dist = d2;
        idx = points[i];
      }
  ...
  if (HITstep == 2)
  {
    numberPoints = holePolygonsX[holePolygonsCount-1].length;
...
function draw_polygon(){
  ...
  {
    ctx.fillStyle = pt_color;
    ctx.fillRect(get_x_vp(points[i])-pt_size,get_y_vp(points[i])-pt_size,2*pt_size,2*pt_size);
  }
  if(numpts > 0){
...
function factor(d,rho) {
  return Math.exp( -0.5 * ( (d/rho) * (d/rho)) );
}

function gaussian_move(idx,ix,iy) {
  var rho = Math.pow(1.25,d_max);

  vx = ix - x[idx];
  vy = iy - y[idx];

  for(var i=0;i<x.length;i++){
    var d = Math.sqrt((x[idx]-x[i])*(x[idx]-x[i]) + (y[idx]-y[i])*(y[idx]-y[i]));
    if(d < d_max ){
      var f = factor(d,rho);
      x[i] = x[i] + f * vx;
      y[i] = y[i] + f * vy;
...
function mousemove_canvas(event){
  ...
  if(polygon_is_closed && selected_idx >= 0){
    ...
    gaussian_move(selected_idx,ix,iy);
...
function initialize_polygon() {
  ...
  var polygon = base_url + '/' + category_name + '/' + gup('polygon');
  var indizes = base_url + '/' + category_name + '/' + gup('points');

  d_max = parseFloat(gup('gaussian'));
  console.log(d_max);

  $.ajaxSetup({ "async" : false });
  $.getJSON( polygon, function( data )
     {
        for(var i = 0; i < data.pointset.length; i++)
        {
          x[numpts] = parseFloat(data.pointset[i].x);
          y[numpts] = parseFloat(data.pointset[i].y);
          numpts = numpts+1;
        }
     });

  $.getJSON( indizes, function( data )
     {
        for(var i = 0; i < data.points.length; i++)
        {
          points[i] = parseInt(data.points[i].i);
        }
     });
